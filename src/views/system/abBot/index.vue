    ...
    <el-card shadow="never">
      <template #header>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button type="primary" plain icon="Plus" @click="handleAdd" v-hasPermi="['system:abBot:add']">新增</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="success" plain icon="Edit" :disabled="single" @click="handleUpdate()" v-hasPermi="['system:abBot:edit']">修改</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete()" v-hasPermi="['system:abBot:remove']">删除</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="warning" plain icon="Download" @click="handleExport" v-hasPermi="['system:abBot:export']">导出</el-button>
          </el-col>
          <right-toolbar v-model:showSearch="showSearch" @queryTable="getList"></right-toolbar>
        </el-row>
      </template>

      <el-table v-loading="loading" :data="abBotList" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['system:abBot:edit']"></el-button>
            </el-tooltip>
            <el-tooltip content="删除" placement="top">
              <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['system:abBot:remove']"></el-button>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
...
const abBotFormRef = ref<ElFormInstance>();
...
const initFormData: AbBotForm = {
  id: undefined,
  botName: undefined,
  abPercentThreshold: undefined,
...
const getList = async () => {
  loading.value = true;
  const res = await listAbBot(queryParams.value);
  abBotList.value = res.rows;
...
const reset = () => {
  ...
  abBotFormRef.value?.resetFields();
}
...
/** 多选框选中数据 */
const handleSelectionChange = (selection: AbBotVO[]) => {
...
const handleAdd = () => {
  reset();
  dialog.visible = true;
  dialog.title = "添加套利机器人";
}

/** 修改按钮操作 */
const handleUpdate = async (row?: AbBotVO) => {
  reset();
  const _id = row?.id || ids.value[0]
  const res = await getAbBot(_id);
  Object.assign(form.value, res.data);
  dialog.visible = true;
  dialog.title = "修改套利机器人";
}
  ...
  abBotFormRef.value?.validate(async (valid: boolean) => {
    if (valid) {
      ...
      if (form.value.id) {
        ...
        await addAbBot(form.value).finally(() =>  buttonLoading.value = false);
      }
...
/** 删除按钮操作 */
const handleDelete = async (row?: AbBotVO) => {
  const _ids = row?.id || ids.value;
  await proxy?.$modal.confirm('是否确认删除套利机器人编号为"' + _ids + '"的数据项？').finally(() => loading.value = false);
  await delAbBot(_ids);
...
/** 导出按钮操作 */
const handleExport = () => {
  proxy?.download('system/abBot/export', {
    ...queryParams.value
  }, `abBot_${new Date().getTime()}.xlsx`)
}
